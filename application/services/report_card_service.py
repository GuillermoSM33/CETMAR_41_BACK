...
def save_parsed_report_cards(
    ...
    for dto in parsed_results:
        ...
        periodo = (_get(dto, "periodo") or "").strip() or "SIN_PERIODO"
        control_num = _get(dto, "numero_control") or "documento_sin_control"
        pdf_path = os.path.join(STORAGE_PATH, f"{control_num}.pdf")

        existing_rc = _get_existing_report_card(db, identity_id=identity.Id, periodo=periodo)

        if existing_rc:
            if not os.path.exists(pdf_path):
                _write_pdf(pdf_path, _try_apply_seal(pdf_content))

            plan_estudios = _get(dto, "plan_estudios") or _get(dto, "generacion")
            carrera = _get(dto, "carrera")

            existing_rc.Plan_Estudios = existing_rc.Plan_Estudios or plan_estudios
            existing_rc.Carrera = existing_rc.Carrera or carrera
            existing_rc.Src_SHA256 = sha256
        ...
        _save_report_items(db, rc_id=rc.Id, items=_get(dto, "uac", []) or [])
        db.commit()
        saved_ids.append(rc.Id)

    return [get_stored_report_card(db, rc_id) for rc_id in saved_ids]

def _get(dto: Any, key: str, default: Any = None) -> Any:

    if dto is None:
        return default

    if isinstance(dto, dict):
        v = dto.get(key, default)
    else:
        v = getattr(dto, key, default)
...
def _get_or_create_identity(db: Session, dto: Any) -> IdentityModel:
    ...
    control = _get(dto, "numero_control")
    if control:
        identity = (
    ...
    alumno = _get(dto, "alumno") or _get(dto, "nombre")
...
def _sync_identity_from_report(dto: Any, identity: IdentityModel) -> None:
    ...
    grupo = _get(dto, "grupo")
    if grupo:
        identity.Grupo = grupo

    turno = _get(dto, "turno")
    if turno:
...
def _get_existing_report_card(db: Session, identity_id: int, periodo: str) -> Optional[ReportCardModel]:
    return (
        db.query(ReportCardModel)
        .filter(
            ReportCardModel.Identity_ID == identity_id,
            ReportCardModel.Periodo == periodo,
        )
        .first()
    )
...
def _create_report_card(db: Session, dto: Any, identity_id: int, sha256: str) -> ReportCardModel:
    ...
    rc = ReportCardModel(
        ...
        Carrera=carrera,
        Avance_Oblig=int(_get(dto, "avance_oblig") or 0),
        Avance_Opt=int(_get(dto, "avance_opt") or 0),
        Avance_Total=int(_get(dto, "avance_total") or 0),
        Promedio=float(_get(dto, "promedio") or 0),
        Src_SHA256=sha256,
    )
    db.add(rc)
    db.flush()
...
def _save_report_items(db: Session, rc_id: int, items: List[Any]) -> None:
    ...
    for item in items:
        clave = _get(item, "clave_uac", None)
        semestre = _get(item, "semestre", None)
        ...
        cal1 = _get(item, "calif1", None)
        cal2 = _get(item, "calif2", None)
        cal3 = _get(item, "calif3", None)
        ...
        rci = ReportCardItemModel(
            ...
            Clave_UAC=key[0],
            Semestre=key[1],
            Nombre=_get(item, "nombre", "") or "",
            Calificacion1=_to_str_or_none(cal1),
            Calificacion2=_to_str_or_none(cal2),
            Calificacion3=_to_str_or_none(cal3),
            Asistencia1=_to_int_or_none(_get(item, "asis1", None)),
            Asistencia2=_to_int_or_none(_get(item, "asis2", None)),
            Asistencia3=_to_int_or_none(_get(item, "asis3", None)),
